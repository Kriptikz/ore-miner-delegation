...
pub async fn test_stake() {
    ...
    println!("MPA PDA: {}", managed_proof_authority.0.to_string());
    println!("MANAGED PROOF TOKEN ACCOUNT: {:?}", miner_token_account);

    // transfer to managed_proof_authority from miner
    // let managed_proof_authority_token_account = get_associated_token_address(&managed_proof_authority.0, &ore_api::consts::MINT_ADDRESS);
    // let ix1 = spl_token::instruction::transfer_checked(&spl_token::id(), &miner_token_account_addr, &ore_api::consts::MINT_ADDRESS, &managed_proof_authority_token_account, &miner.pubkey(), &[&miner.pubkey()], miner_token_balance, ore_api::consts::TOKEN_DECIMALS).unwrap();
    // let mut tx =
    //     Transaction::new_with_payer(&[ix1], Some(&miner.pubkey()));
